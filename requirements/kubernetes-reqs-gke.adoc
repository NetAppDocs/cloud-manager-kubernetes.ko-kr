...
이 항목에서는 _Kubernetes cluster_를 사용합니다. 여기서 구성은 GKE 및 자체 관리되는 Kubernetes 클러스터의 경우 동일합니다. 클러스터 유형은 구성이 다른 곳에서 지정됩니다.
...
* https://docs.netapp.com/us-en/cloud-manager-connector/task-creating-connectors-gcp.html["Cloud Manager에서 커넥터를 생성합니다"^] (권장)
* https://docs.netapp.com/us-en/cloud-manager-connector/task-installing-linux.html["기존 Linux 호스트에 커넥터를 설치합니다"^]
...
. 다음 텍스트가 포함된 YAML 파일을 생성합니다. 'Subjects:kind:' 변수를 사용자 이름으로 바꾸고 'Subjects:user:'를 인증된 서비스 계정의 고유 ID로 바꿉니다.
+
[source, yaml]
----
